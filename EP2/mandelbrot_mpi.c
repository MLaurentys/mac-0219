...
void init(int argc, char *argv[]) {
    if(argc < 6){
        printf("usage: ./mandelbrot_mpi c_x_min c_x_max c_y_min c_y_max image_size n_threads \n");
...
void compute_mandelbrot(){
    ...
    int numtasks, taskid, chunksize, leftover;
    ...
    int rows_per_thread = image_size / numtasks;
    int rems = image_size - rows_per_thread * numtasks;

    if (taskid == MASTER) {
      printf("mpi_mm has started with %d tasks.\n",numtasks);
    }
    printf("Task id = %d\n", taskid);
    int toAdd = (rems - taskid > 0) ? 1 : 0;
    int tmp = (rems > taskid) ? taskid : rems;
    int begin_y = taskid * rows_per_thread + tmp;
    int end_y = begin_y + toAdd + rows_per_thread;
    printf("doing %d ->= %d\n", begin_y, end_y);
    ...
    for(int y = begin_y; y < end_y; ++y) {
        ...
        for(i_x = 0; i_x < i_x_max; i_x++){
            ...
            update_rgb_buffer(iteration, i_x, y);
        };
    };
